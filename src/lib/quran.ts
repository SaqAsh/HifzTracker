...
export type LessonAssignment = {
  assignment_type: null | 'lesson' | 'revision';
...
export const SURAHS: Surah[] = [
  ...
  { number: 42, name: 'Ash-Shuraa', ayahCount: 53 },
  ...
  { number: 93, name: 'Ad-Duhaa', ayahCount: 11 },
...
export const SURAH_OPTIONS = SURAHS.map((surah) => ({
  label: `${surah.number}. ${surah.name}`,
  value: String(surah.number),
}));

export const JUZ_OPTIONS = Array.from({ length: 30 }, (_, index) => ({
  label: `Juz ${index + 1}`,
  value: String(index + 1),
}));

export const HIZB_OPTIONS = Array.from({ length: 60 }, (_, index) => ({
  label: `Hizb ${index + 1}`,
  value: String(index + 1),
}));

/** Returns surah metadata by 1-based surah number. */
export function getSurah(number: null | number): null | Surah {
  return SURAHS.find((surah) => surah.number === number) ?? null;
}

/** Returns a compact assignment summary for lesson cards and student portal. */
export function formatAssignmentSummary(assignment: LessonAssignment): string {
  if (assignment.assignment_type === 'revision') {
    if (assignment.revision_mode === 'juz' && assignment.revision_juz_from) {
      return `Revision: Juz ${formatRange(assignment.revision_juz_from, assignment.revision_juz_to)}`;
    }

    if (assignment.revision_mode === 'hizb' && assignment.revision_hizb_from) {
      return `Revision: Hizb ${formatRange(assignment.revision_hizb_from, assignment.revision_hizb_to)}`;
    }

    if (
      assignment.revision_mode === 'surah_range' &&
      assignment.revision_surah_from
    ) {
      const from = getSurah(assignment.revision_surah_from)?.name ?? 'Unknown';
      const to = getSurah(assignment.revision_surah_to)?.name ?? from;
      return from === to ? `Revision: ${from}` : `Revision: ${from} to ${to}`;
    }

    return 'Revision';
  }

  const surah = getSurah(assignment.lesson_surah_number);

  if (!surah) {
    return 'Lesson';
  }

  const ayahText = formatAyahRange(
    assignment.lesson_ayah_from,
    assignment.lesson_ayah_to,
  );

  return ayahText
    ? `Lesson: ${surah.name}, ayah ${ayahText}`
    : `Lesson: ${surah.name}`;
}

function formatRange(from: number, to: null | number): string {
  return to && to !== from ? `${from}-${to}` : String(from);
}

function formatAyahRange(from: null | number, to: null | number): string {
  if (!from && !to) {
    return '';
  }
